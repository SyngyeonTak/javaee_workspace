...
<%
	...
	*/
	...
	//업로드 객체를 생성해주는 팩토리 객체: 주로 설정을 담당(서버의 저장 경로, 파일의 용량 제한...)
	...
	//FileItem은 클라이언트의 전송 정보 하나 하나를 의미한다. 즉, html에서의 input text, file 컴포넌트 등을 의미
	//우리의 경우 input type="text"가 FileItem에 담기고
	//우리의 경우 input type="file"가 FileItem에 담기고
	request.setCharacterEncoding("utf-8");
	List<FileItem> items = upload.parseRequest(request);//upload 인스턴스에게 클라이언트의 request 정보를 전달한다
	
	for(FileItem item : items){
		//out.print("저는 "+item.getFieldName());
		
		//반복문으로 처리되다 보니, 파일만 따로 처리를 하려면, 구분 로직이 필요함...
		if(!item.isFormField()){//텍스트박스가 아닌 것만 업로드 처리!! FormField <= TextBox
