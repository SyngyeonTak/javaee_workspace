		...
		<tr>
			<td colspan="4" style="text-align:center">
					<%if(pager.getFirstPage() >1){ %>
						<a href = "/board/list.do?currentPage=<%=pager.getFirstPage()-1%>">◀</a>					
					<%}else{ %>
				...
				<%for(int i = pager.getFirstPage(); i<=pager.getLastPage(); i++){ %>
					...
					<a href = "/board/list.do?currentPage=<%=i%>" <%if(pager.getCurrentPage()==i) {%>class = "pageNum"<%} %>><%=i %></a>
				<%} %>
					<%if(pager.getLastPage() < pager.getTotalPage()) {%>
						<a href = "/board/list.do?currentPage=<%=pager.getLastPage()+1%>">▶</a>
					<%}else{ %>
